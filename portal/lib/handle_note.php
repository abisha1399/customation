...
if(isset($GLOBALS['enable_attachmail_doc'])&&$GLOBALS['enable_attachmail_doc']==true){
    ...
    {
        $path="mail_documents/".$_SESSION['pid'];
        if(!file_exists($path))
        {
            mkdir($path, 0777, true);
        }
    ...
    $target = $path. basename($_FILES['attach_file']['name']);
    $image = ($_FILES['attach_file']['name']);
    if(!empty($image))
    {
        $newname=rand(10,1000).'_'.$image;
        move_uploaded_file($_FILES['attach_file']['tmp_name'], $path.$newname);
    }
}
