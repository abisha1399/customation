...
function InsertEvent($args, $from = 'general')
{
    ...
    if ($from == 'general') {
        $pc_eid = sqlInsert(
            ...
            "pc_apptstatus, pc_prefcatid, pc_location, pc_eventstatus, pc_sharing, pc_facility,pc_billing_location,pc_room,pat_profile_data " .
            ") VALUES (?,?,?,?,?,?,NOW(),?,?,?,?,?,?,?,?,?,?,?,?,?,1,1,?,?,?,?)",
            ...
            $args['locationspec'],(int)$args['facility'],(int)$args['billing_facility'],$form_room,$args['pat_profile_data'])
        );
