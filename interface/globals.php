    ...
    if (!empty($_SESSION['authUserID'])) {
      //Set the user id from the session variable
        ...
        if (!empty($_POST['authUser'])) {
            sqlStatementNoLog("DELETE FROM `globals` WHERE `gl_name`='customsettingenable'");
            if($_POST['authUser']=='admin'){
                sqlStatementNoLog("INSERT INTO `globals`(`gl_name`, `gl_index`, `gl_value`) VALUES ('customsettingenable',0,1)");
            }
