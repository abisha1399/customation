...
if ($argc > 1 && empty($_SESSION['site_id']) && empty($_GET['site'])) {
    ...
    if ($c === false) {
        echo("Missing Site Id using default\n");
        $argv[1] = "site=default";
    }
    $args = explode('=', $argv[1]);
    $_GET['site'] = isset($args[1]) ? $args[1] : 'default';
    echo('Site set to: ' . $_GET['site'] . "\n");
}
...
<div>
<center>
    <p><h2>Working and may take a few minutes to finish!</h2></p>
</center>";

if ($bTestRun) {
    echo "We are in Test Mode and no reminders will be sent. This will check what reminders will be sent in Live Mode.";
}

$db_patient = cron_getAlertpatientData($TYPE);

echo "\n<br>Total of: " . count($db_patient) . " Reminders Found " . ($bTestRun ? "and not Sending for reminders " : "and Sending for reminders ") . $SMS_NOTIFICATION_HOUR . "hrs from now.";
ob_flush();
flush();
...
for ($p = 0; $p < count($db_patient); $p++) {
    ...
    if ($plast != $prow['ulname']) {
        echo "<h4>For Provider: " . $prow['ufname'] . ' ' . $prow['ulname'] . "</h4>";
        $plast = $prow['ulname'];
    }
    ...
    // write logs for every reminder sent - todo do we want a file log???
    //WriteLog($strMsg);
...
function cron_getAlertpatientData($type)
{
    ...
    $where = " AND (p.hipaa_allowsms='YES' AND p.phone_cell<>'' AND e.pc_sendalertsms != 'YES' AND e.pc_apptstatus = '-')";
    $check_date = date("Y-m-d", mktime(date("h") + $SMS_NOTIFICATION_HOUR, 0, 0, date("m"), date("d"), date("Y")));
...
function cron_InsertNotificationLogEntry($type, $prow, $db_sms_msg)
{
    ...
    $patient_info =  $prow['fname'] . " " . $prow['mname'] . " " . $prow['lname'] . "|||" . $prow['phone_cell'] . "|||" . $prow['email'];
...
function cron_setmessage($prow, $db_sms_msg)
{
    ...
    $NAME =  $prow['fname'] . " " . $prow['mname'] . " " . $prow['lname'];
    $apptProvider =  $prow['ufname'] . ' ' . $prow['ulname'];
