...
function initializePassword($username, $userid, &$password)
{
    ...
    sqlStatement($passwordSQL, $params);
    return $hash;
}
...
function purgeCompatabilityPassword($username, $userid)
{
    $purgeSQL = " UPDATE " . TBL_USERS
        ...
        . " WHERE " . COL_UNM . "=? "
        . " AND " . COL_ID . "=?";
    sqlStatement($purgeSQL, array($username, $userid));
}
...
function confirm_user_password($username, $password)
{
    ...
    $getUserSecureSQL = "SELECT * FROM `patient_access_onsite` where portal_username=?";
    $userSecure = sqlQuery($getUserSecureSQL, array($username));
    if (is_array($userSecure)) {
        $phash = oemr_password_hash($password, $userSecure['portal_salt']);
        if ($phash == $userSecure['portal_pwd']) {
            return true;
        }
    }
