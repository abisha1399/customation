...
// print_r($note);die;
    ...
    if ($id && $id != 0) {
    sqlStatement("UPDATE form_current_withdrawal_signs SET check1=?,check2=?,check3=?,check4=?,check5=?,check6=?,check7=?,check8=?,check9=?,check10=?,check11=?,check12=?,check13=?,check14=?,check15=?,check16=?,check17=?,check18=?,check19=?,check20=?,check21=?,check22=?,check23=?,check24=?,check25=?,check26=?,check27=?,check28=?,check29=?,check30=?,check31=?,check32=?,check33=?,check34=?,check35=?,check36=?,check37=?,check38=?, check39=?,check40=?,check41=?,check42=?,check43=?,check44=?,check45=?,check46=?,check47=?,check48=?,check49=?,check50=?,check51=?,check52=?,check53=?,check54=?,check55=?,check56=?,check57=?,check58=?,check59=?,check60=?,check61=?,check62=?,check63=?,check64=?,check65=?,check66=?,check67=?,check68=?,check69=?,check70=?,check71=?,check72=?,check73=?,check74=?,pain=?,intensity=?,acceptable=?,characteristic=?,onset=?,relieves=?,causes=?,effects=?,check75=?,check76=?,treatment=?,intervention=?,note=? WHERE id = ?",
    array($check1,$check2,$check3,$check4,$check5,$check6,$check7,$check8,$check9,$check10,$check11,$check12,$check13,$check14,$check15,$check16,$check17,$check18,$check19,$check20,$check21,$check22,$check23,$check24,$check25,$check26,$check27,$check28,$check29,$check30,$check31,$check32,$check33,$check34,$check35,$check36,$check37,$check38,$check39,$check40,$check41,$check42,$check43,$check44,$check45,$check46,$check47,$check48,$check49,$check50,$check51,$check52,$check53,$check54,$check55,$check56,$check57,$check58,$check59,$check60,$check61,$check62,$check63,$check64,$check65,$check66,$check67,$check68,$check69,$check70,$check71,$check72,$check73,$check74,$pain,$intensity,$acceptable,$characteristic,$onset,$relieves,$causes,$effects,$check75,$check76,$treatment,$intervention,$note,$id));
}else 
{
    $newid = sqlInsert("INSERT INTO form_current_withdrawal_signs(pid,encounter,check1,check2,check3,check4,check5,check6,check7,check8,check9,check10,check11,check12,check13,check14,check15,check16,check17,check18,check19,check20,check21,check22,check23,check24,check25,check26,check27,check28,check29,check30,
    ...
    addForm($encounter, "Current Withdrawal Signs/Symptoms", $newid, "current_withdrawal_signs_symptoms",  $_SESSION["pid"], $userauthorized);
}
