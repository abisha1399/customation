...
$tenovi_user_data_results=[];
$tenovi_user_data=sqlStatement("SELECT pid FROM tenovi_data WHERE hwi_id!='' GROUP BY pid");
while($tenovi_results = sqlFetchArray($tenovi_user_data)){
    $tenovi_user_data_results[]=$tenovi_results;
}
$startdate = date(("Y-m-d")).'T'.date('H', strtotime(' -1 hours')).':00:00Z';
$startdate='2023-02-18T20:00:00Z';
if(!empty($tenovi_user_data_results))
{
    $pid=$tenovi_user_data_results['pid'];
    ...
    if($tenovi_data_count>0)
    {
        ...
        if(!empty($all_measurement_data))
        {
                ...
                echo 'tenovi data '.$pid.': <br>'.'<pre>';print_r($all_measurement_data);
        ...
        echo "No device to get data for ".$pid."";
    }
}
else{
    echo 'patiet no have tenovi data';
}
// $tenovi_data=sqlStatement("SELECT * FROM tenovi_data WHERE hwi_id!=''");
// $tenovi_data_count=sqlNumRows($tenovi_data);
