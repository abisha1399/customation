...
if(isset($data['status']['status_code'])==200){ 
   ...
   if(!empty($iglucose_data)){
       foreach($iglucose_data as $iglucose_val){
           ...
           if(empty($id))
           {
               sqlStatement("INSERT INTO api_vitals_data(reading_time,pulse,systolic,diastolic,blood_glucose_mmol,blood_glucose,reading_date,pid,reading_type,api_type) VALUES (?,?,?,?,?,?,?,?,?,?)",array($reading_time,$pulse,$systolic,$diastolic,$blood_glucose_mmol,$blood_glucose_mgdl,$reading_date,$pid,$reading_type,'smart_meter_api'));
               //  $sql_update= "UPDATE api_vitals_data SET reading_time=?,pulse=?,systolic=?,diastolic=?,blood_glucose_mmol=?,blood_glucose_mgdl=?,reading_date=?,pid=?,reading_type=? where id=?";
